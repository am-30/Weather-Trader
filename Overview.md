...
│   │                        # CDF interpolation
│   └── edge_computer.py     # Shared MC→edge computation (EdgeRow
│                            # dataclass); used by UI + paper trader
├── execution/
│   ├── trader.py            # Kill switch → MC → Kelly → orders
│   └── paper_trader.py      # Paper trading simulator: 10 AM entry,
│                            # limit-sell exit, settlement close
├── calibration/
...
intraday_snapshots, trade_logs, paper_trade_positions

paper_trade_positions: simulated positions from paper trading
  simulator. Separate from trade_logs (no dry_run flag mixing).
  Key fields: position_id (UUID PK), target_date, market_ticker,
  action, kalshi_strike, entry_price_cents, contracts, cost_usd,
  fair_value_prob, edge_at_entry, budget_mode, status
  (open|limit_sell_closed|settled_win|settled_loss),
  exit_price_cents, pnl_cents, pnl_usd, official_high_f,
  settlement_win. Index on (target_date, entry_at_utc DESC).
...
### Paper Trading Parameters (settings.py)

paper_budget_mode: 'flat' (default) | 'kelly'
  flat: $10/day (paper_daily_budget) split equally among qualifying
    signals; balance accumulates and can go negative.
  kelly: fractional Kelly from paper_initial_bankroll ($100 default).
paper_entry_max_ask_cents: 50 — skip entry if ask >= this value
paper_limit_sell_cents: 75 — close position when bid reaches this

LIMIT SELL LOGIC (paper_trader.check_limit_sell_exits):
  BUY_YES: trigger when yes_bid >= paper_limit_sell_cents
    (uses BID, not ask — bid is the actual sell price)
  BUY_NO: trigger when (100 - yes_ask) >= paper_limit_sell_cents
    (100 - yes_ask = NO bid = price you receive selling NO)
  Large spreads are handled correctly: bid is always used, not ask.

Scheduler hooks:
  Job 9 (10:00 AM ET cron): job_paper_trade_10am() → run_paper_entry_10am()
  job_evaluate_trade() (every 5 min): also calls check_limit_sell_exits()
  job_confirm_settlement() (10:05 AM ET): also calls run_paper_settlement_close()
...
- [x] March 29: ou_max_stationary_std raised 1.0 → 2.0 (cap was suppressing
              ...
              92/92 tests passing
- [x] April 5, 2026: Paper trading simulator
              quant/edge_computer.py: shared EdgeRow dataclass +
              compute_edge_table() extracted from UI; used by both
              Trading Desk display and paper trader scheduler job.
              execution/paper_trader.py: run_paper_entry_10am()
              (10AM cron, ask < 50¢ filter, $10/day flat budget),
              check_limit_sell_exits() (every 5 min, bid-based
              trigger at 75¢), run_paper_settlement_close() (after
              NWS CLI confirmation). paper_trade_positions table
              with full P&L tracking. Trading Desk UI: date-range
              filter, cumulative balance chart (blue line + daily
              P&L bars), open positions table, trade history.
              db: get_paper_trade_daily_series() for day-by-day
              balance series; get_paper_trade_summary(since, until)
              for filtered aggregate stats.
